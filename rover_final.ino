...
const float AVOIDANCE_ANGLE = 38;
const float AVOIDANCE_DELAY = 1250;
...
// Record distance for going back
long dist;
...
void state5()
{
  if (ObjDistance() > 5 && FLDistance() > 1 && FRDistance() > 1){
    forward();
  }
  else {
    stop();
...
void avoidLeft(){
  turnQuantity(-AVOIDANCE_ANGLE * 1.07);
  forward();
